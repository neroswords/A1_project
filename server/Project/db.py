...
def new_bot(username):
    bot_collection.insert_one({'username': username})

def update_connect(username, ch_sc,ch_ac_tk,basic_id,pfa_tk,vf_tk):
    
    bot_collection.update_one({ "username": username },{ "$set": { "Channel_secret": ch_sc, "Channel_access_token": ch_ac_tk, "basic_id": basic_id, "page_facebook_access_token": pfa_tk, "VERIFY_TOKEN": vf_tk} })
